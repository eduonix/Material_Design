...
public class NavigationLiveoAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(position == 2)
		{
			holder.layoutDados.setPadding(50, 0, 0, 0);	
		}
		...
		if (!item.isHeader) {			
			if (item.checked) {
				// commented for selector               convertView.setBackgroundResource((!item.removeSelector ? ( mNewSelector == 0 ? R.drawable.selector_check_item_navigation : mNewSelector) : R.drawable.selector_no_check_item_navigation));
			} else {
				//  commented for selector                convertView.setBackgroundResource(R.drawable.selector_no_check_item_navigation);
			}
		}else{
			//    commented for selector         convertView.setBackgroundResource(R.drawable.selector_no_check_item_navigation);
		}
